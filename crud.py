...
async def create_orders_and_products(session: AsyncSession):
    ...
    mouse = await create_product(
        session,
        "Mouse",
        description="Great Gamimg Mouse",
        price=123,
    )
...
async def demo_get_orders_with_products_with_associations(session: AsyncSession):
    orders = await get_orders_with_products_associations(session)
    ...
    for order in orders:
        ...
        for (
            ...
            print(
                "-",
                order_prod_details.product.id,
                order_prod_details.product.name,
                order_prod_details.product.price,
                "qty:",
                order_prod_details.count,
            )
...
async def main_relations(session: AsyncSession):
    ...
    await create_user(session=session, username="Gate")
